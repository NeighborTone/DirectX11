...
void RigidBody::SetRotation(Vec3& angle)
{
	...
	dMatrix3 R;
	dRFromAxisAndAngle(R, 1.0, 0.0, 0.0, DirectX::XMConvertToRadians(angle.x));	//x軸周りに[rad]回転
	dRFromAxisAndAngle(R, 0.0, 1.0, 0.0, DirectX::XMConvertToRadians(angle.y));	//y軸周りに[rad]回転
	dRFromAxisAndAngle(R, 0.0, 0.0, 1.0, DirectX::XMConvertToRadians(angle.z));	//z軸周りに[rad]回転
